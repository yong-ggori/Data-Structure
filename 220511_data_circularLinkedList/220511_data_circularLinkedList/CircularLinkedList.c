...
void insertFirstNode(linkedList_h* CL, char* x) {
	...
	if (CL->head == NULL) {		// 원형 연결 리스트가 공백 리스트일 경우
		CL->head = newNode;		// 새 노드를 리스트의 헤드로 연결
		newNode->link = newNode;
	}
	else {						// 원형 연결 리스트가 공백 리스트가 아닐 경우
		temp = CL->head;
...
void deleteNode(linkedList_h* CL, listNode* old) {
	...
	if (CL->head == NULL) return;		// 공백 리스트일 경우 삭제 연산 중단
	if (CL->head->link == CL->head) {	// 리스트에 노드가 한 개 있는 경우
		free(CL->head);					// 첫 번째 노드의 메모리를 해제하고
		CL->head = NULL;				// 리스트 시작 포인터를 NULL로 설정
		return;
	}

	else if (old == NULL) return;	// 삭제할 노드가 없는 경우 강제 연산 중단
	else {
